...
def readfromfile():
    with open("SQA 2025 PRACTICE ASSIGNMENT/orders.txt","r") as readfile:  #open orders.txt
        ...
        while line:
            ...
            counter += 1    
            line=readfile.readline().rstrip("\n")                 #increments counter by one to add to next position in array
        return orders
...
def findpositionofcustomer(orders):
    ...
    while (position == -1 and index < len(orders)):     # 2.4 While position is -1 and index is less than the length of the array
        if (orders[index].date[3:6]==month and orders[index].rating==5):     # 2.5 If current month is equal to searched month and current rating is 5 then   #and orders[index].rating==5
...
def WriteDetailsOfWinningCustomer(orders,position):
    with open("SQA 2025 PRACTICE ASSIGNMENT/winningCustomer.txt", "w") as writefile: #3.1 Open new file 'winningCustomer.txt'
...
def countOption(orders, target):
    ...
    for index in range (len(orders)):
        if (orders[index].options== target):
            count += 1
    return count
...
delivercount = countOption(orders, "Delivery")
collectorcount=countOption(orders, "Collection")

print("there were " + str(delivercount) + " deliveries")
print("there were " + str(collectorcount) + " collections")